---
name: tinji
description: Helps implement tinji UI components correctly. Use when building UIs with tinji primitives (buttons, dialogs, selects, forms, menus, tabs, inputs, toasts, etc.), migrating from shadcn/Radix to tinji/Base UI, composing trigger-based overlays, or troubleshooting tinji component behavior. Covers imports, accessibility, Tailwind styling, and common pitfalls.
compatibility: Requires Tailwind CSS v4 and @base-ui/react. Designed for React projects using the tinji component registry.
license: MIT
metadata:
  author: aussieljk
---
...
tinji ui is a component library built on Base UI with a shadcn-like developer experience plus a large particle catalog.

## What this skill is for

Use this skill to:

- pick the right tinji primitive(s) for a UI task
- write correct tinji usage code (imports, composition, props)
...
- tinji components docs: `apps/ui/content/docs/components/*.mdx`
  - `https://github.com/aussieljk/tinji-ui/tree/main/apps/ui/content/docs/components`
- tinji particle examples: `apps/ui/registry/default/particles/p-*.tsx`
...
- docs map for agents: `https://ui.tinji.dev/ui/llms.txt`

## Out of scope

- Maintaining tinji monorepo internals/build pipelines.
...
3. Follow tinji naming and APIs from docs exactly.
4. Keep examples accessible and production-realistic.
5. Prefer concise code that mirrors tinji docs/particles conventions.
6. Assume Tailwind CSS v4 conventions in tinji examples and setup guidance.

## Critical usage rules

Always apply before returning tinji code:

- Do not invent tinji APIs. Verify against component docs first.
...
- Prefer styled tinji exports first; use `*Primitive` exports only when custom composition/styling requires it.
...
- `./references/rules/migration.md` — shadcn/Radix to tinji/Base UI migration patterns
...
3. Select primitives from tinji docs first; avoid custom fallback unless needed.
...
```bash
npx shadcn@latest add @tinji/<component>
```
...
- imports and props match tinji docs
