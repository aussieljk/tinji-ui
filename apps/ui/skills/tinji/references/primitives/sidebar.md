...
```bash
npx shadcn@latest add @tinji/sidebar
```
...
- For link items, use `render` composition: `<SidebarMenuButton render={<a href="..." />}>`. Do not use `asChild` -- sidebar follows the same `render` pattern as all other tinji primitives.
