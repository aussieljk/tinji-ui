...
```bash
npx shadcn@latest add @tinji/form
```
...
## Patterns from tinji/Base UI forms
...
- **Textarea integration**: use tinji `Textarea` directly inside `Field`; it already integrates with Base UI field control semantics, so `FieldControl render={...}` is not required for standard textarea usage.
