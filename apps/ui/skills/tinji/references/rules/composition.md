# Composition Rules (tinji)

Use this when composing complex UI from tinji primitives and particles.
...
- Use tinji/Base UI trigger APIs consistently (typically `render`-based composition).
...
- Mixing APIs from other ecosystems (`asChild`-only mental model) without checking tinji equivalents.
