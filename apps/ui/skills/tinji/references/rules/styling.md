...
Use this guide when writing or updating tinji primitives, particles, and docs examples.
...
- Use Tailwind v4 syntax and conventions in tinji examples and snippets.
- **Do not replace `--alpha()` with `color-mix()` or `rgba()`.** `--alpha()` is a valid Tailwind v4 theme function used throughout tinji token definitions (e.g. `--alpha(var(--color-black) / 8%)`). It is processed by Tailwind at build time — it is not invalid CSS.

## tinji-specific Expectations
...
- Preserve tinji token architecture. Do not replace tinji semantic variables with ad-hoc color classes/tokens.
...
tinji components use three CSS custom properties for typography:
...
**CLI setup (recommended):** `npx shadcn@latest init @tinji/style` automatically installs `@tinji/fonts` — Inter for `--font-sans` and `--font-heading`, Geist Mono for `--font-mono` — via `registry:font` items and configures them in `layout.tsx`.

**Manual / custom font setup:** When using `next/font`, the `variable` option must match tinji expectations exactly:
...
**Common pitfall:** Next.js starters default to `--font-geist-sans` and `--font-geist-mono`, which do not match tinji's `--font-sans` / `--font-mono`. Fonts will silently fall back to system UI. Always rename the variables or remap them.
...
3. Any icon sizing/opacities violating tinji conventions?
